...
def venues():
    ...
    if Venue.query.count() == 0:
       ...
       # A list of dictionaries, where city, state, and venues are dictionary keys
...
def show_venue(venue_id):
    ...
    if not venue:
        # If not in database, check if it's one of our test venues
        test_venues = {
            1: {
                "id": 1,
                "name": "The Musical Hop",
        ...
        if venue_id in test_venues:
            data = test_venues[venue_id]
    ...
    # For Park Square Live Music & Coffee (venue_id=3), use test data if in database
    if venue.id == 2:
        data["past_shows"] = [{
            "artist_id": 2,
            "artist_name": "Matt Quevedo",
        ...
        data["upcoming_shows_count"] = 3
    

    else:
        # For other venues, use database data
        for show in shows:
            show_info = {
